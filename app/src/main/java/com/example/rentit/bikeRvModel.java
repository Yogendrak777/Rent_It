package com.example.rentit;

public class bikeRvModel {
    String bikeAddress;
    String bikeAdvance;
    String bikeAge;
    String bikeArea;
    String bikeDescription;
    String bikeMileage;
    String bikeModel;
    String bikeRent;
    String bikeServiceDate;
    String bikeType;
    String bikeUrl1;
    String bikeUrl2;
    String bikeUrl3;
    String bikeUrl4;
    String type;

    public bikeRvModel(){}

    public bikeRvModel(String bikeAddress, String bikeAdvance, String bikeAge, String bikeArea, String bikeDescription, String bikeMileage, String bikeModel, String bikeRent, String bikeServiceDate, String bikeType, String bikeUrl1, String bikeUrl2, String bikeUrl3, String bikeUrl4, String type) {
        this.bikeAddress = bikeAddress;
        this.bikeAdvance = bikeAdvance;
        this.bikeAge = bikeAge;
        this.bikeArea = bikeArea;
        this.bikeDescription = bikeDescription;
        this.bikeMileage = bikeMileage;
        this.bikeModel = bikeModel;
        this.bikeRent = bikeRent;
        this.bikeServiceDate = bikeServiceDate;
        this.bikeType = bikeType;
        this.bikeUrl1 = bikeUrl1;
        this.bikeUrl2 = bikeUrl2;
        this.bikeUrl3 = bikeUrl3;
        this.bikeUrl4 = bikeUrl4;
        this.type = type;
    }

    public String getBikeAddress() {
        return bikeAddress;
    }

    public void setBikeAddress(String bikeAddress) {
        this.bikeAddress = bikeAddress;
    }

    public String getBikeAdvance() {
        return bikeAdvance;
    }

    public void setBikeAdvance(String bikeAdvance) {
        this.bikeAdvance = bikeAdvance;
    }

    public String getBikeAge() {
        return bikeAge;
    }

    public void setBikeAge(String bikeAge) {
        this.bikeAge = bikeAge;
    }

    public String getBikeArea() {
        return bikeArea;
    }

    public void setBikeArea(String bikeArea) {
        this.bikeArea = bikeArea;
    }

    public String getBikeDescription() {
        return bikeDescription;
    }

    public void setBikeDescription(String bikeDescription) {
        this.bikeDescription = bikeDescription;
    }

    public String getBikeMileage() {
        return bikeMileage;
    }

    public void setBikeMileage(String bikeMileage) {
        this.bikeMileage = bikeMileage;
    }

    public String getBikeModel() {
        return bikeModel;
    }

    public void setBikeModel(String bikeModel) {
        this.bikeModel = bikeModel;
    }

    public String getBikeRent() {
        return bikeRent;
    }

    public void setBikeRent(String bikeRent) {
        this.bikeRent = bikeRent;
    }

    public String getBikeServiceDate() {
        return bikeServiceDate;
    }

    public void setBikeServiceDate(String bikeServiceDate) {
        this.bikeServiceDate = bikeServiceDate;
    }

    public String getBikeType() {
        return bikeType;
    }

    public void setBikeType(String bikeType) {
        this.bikeType = bikeType;
    }

    public String getBikeUrl1() {
        return bikeUrl1;
    }

    public void setBikeUrl1(String bikeUrl1) {
        this.bikeUrl1 = bikeUrl1;
    }

    public String getBikeUrl2() {
        return bikeUrl2;
    }

    public void setBikeUrl2(String bikeUrl2) {
        this.bikeUrl2 = bikeUrl2;
    }

    public String getBikeUrl3() {
        return bikeUrl3;
    }

    public void setBikeUrl3(String bikeUrl3) {
        this.bikeUrl3 = bikeUrl3;
    }

    public String getBikeUrl4() {
        return bikeUrl4;
    }

    public void setBikeUrl4(String bikeUrl4) {
        this.bikeUrl4 = bikeUrl4;
    }

    public String getType() {
        return type;
    }

    public void setType(String type) {
        this.type = type;
    }
}
